...
- CRÍTICO — cuándo existe "self": SOLO en on_collision (self = entidad del tag "a", the_collider = entidad del tag "b"), on_spawn y on_destroy. En triggers GLOBALES (every_frame, every, on_input, on_cross, on_draw) NO hay self. Si en un trigger global usás for:{"s":"self"} la regla NO selecciona nada y NO corre. Para que el JUGADOR actúe (ej. disparar al apretar una tecla, aletear), usá SIEMPRE for:{"s":"with_tag","tag":"bird"} (NO {"s":"self"}). Dentro de esa regla, "self" ya es el bird y podés usar self.pos, etc.
